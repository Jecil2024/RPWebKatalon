...
// Apply Chrome arguments dynamically
...
// --------------------- DEFINE ELEMENTS ---------------------
TestObject gasSupplySwitch = new TestObject("gasSupplySwitch")
gasSupplySwitch.addProperty("xpath", ConditionType.EQUALS, "//*[@id='requeststab']/div/table[3]/tbody/tr[13]/td[2]/label/div")
...
// --------------------- SCROLL PANEL TO FIND GAS SUPPLY SWITCH ---------------------
...
for (int i = 0; i < 15; i++) {
	try {
		WebElement toggleElement = driver.findElement(By.xpath("//*[@id='requeststab']/div/table[3]/tbody/tr[13]/td[2]/label/div"))
...
if (!found) {
	KeywordUtil.markFailed("❌ Gas Supply switch not visible after scrolling.")
	return
}

// --------------------- TOGGLE SWITCH ---------------------
WebUI.scrollToElement(gasSupplySwitch, 5)
WebUI.delay(2)

WebElement switchElement = WebUI.findWebElement(gasSupplySwitch)
boolean isSwitchOn = switchElement.getAttribute("class").contains("switchOn")

if (!isSwitchOn) {
	WebUI.click(gasSupplySwitch)
	KeywordUtil.logInfo("🔘 Gas Supply switch was OFF — turned ON now.")
	isSwitchOn = true
} else {
	WebUI.click(gasSupplySwitch)
	KeywordUtil.logInfo("🔘 Gas Supply switch was ON — turned OFF now.")
	isSwitchOn = false
}
...
// Expected keyword
String gasCertText = "gas safety certificate for the property;"

// --------------------- VALIDATION ---------------------
if (isSwitchOn) {
	if (pageSource.contains(gasCertText)) {
		KeywordUtil.markPassed("✅ PASS: Gas Supply switch is ON and 'Gas Safety Certificate for the Property;' FOUND in TA.")
	} else {
		KeywordUtil.markFailed("❌ FAIL: Gas Supply switch is ON but 'Gas Safety Certificate for the Property;' NOT found in TA.")
	}
} else {
	if (!pageSource.contains(gasCertText)) {
		KeywordUtil.markPassed("✅ PASS: Gas Supply switch is OFF and 'Gas Safety Certificate for the Property;' NOT found in TA (as expected).")
	} else {
		KeywordUtil.markFailed("❌ FAIL: Gas Supply switch is OFF but 'Gas Safety Certificate for the Property;' was found in TA.")
	}
}
...
WebUI.closeBrowser()
KeywordUtil.logInfo("🎯 Test completed successfully for Gas Supply toggle.")
