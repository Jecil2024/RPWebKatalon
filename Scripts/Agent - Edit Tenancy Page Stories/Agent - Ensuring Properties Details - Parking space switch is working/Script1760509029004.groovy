...
// --------------------- DEFINE ELEMENTS ---------------------
TestObject parkingSwitch = new TestObject("parkingSwitch")
parkingSwitch.addProperty("xpath", ConditionType.EQUALS, "//*[@id='requeststab']/div/table[3]/tbody/tr[2]/td[4]/label/div")
...
// --------------------- SCROLL PANEL TO FIND PARKING SWITCH ---------------------
...
for (int i = 0; i < 15; i++) {
	try {
		WebElement toggleElement = driver.findElement(By.xpath("//*[@id='requeststab']/div/table[3]/tbody/tr[2]/td[4]/label/div"))
...
if (!found) {
	KeywordUtil.markFailed("❌ Parking space switch not visible after scrolling.")
	return
}

// --------------------- TOGGLE SWITCH ---------------------
WebUI.scrollToElement(parkingSwitch, 5)
WebUI.delay(2)

WebElement switchElement = WebUI.findWebElement(parkingSwitch)
boolean isSwitchOn = switchElement.getAttribute("class").contains("switchOn")

if (!isSwitchOn) {
	WebUI.click(parkingSwitch)
	KeywordUtil.logInfo("🔘 Parking space switch was OFF — turned ON now.")
	isSwitchOn = true
} else {
	WebUI.click(parkingSwitch)
	KeywordUtil.logInfo("🔘 Parking space switch was ON — turned OFF now.")
	isSwitchOn = false
}
...
boolean hasParkingText = pageSource.toLowerCase().contains("parking space")

if (isSwitchOn) {
	if (hasParkingText) {
		KeywordUtil.markPassed("✅ PASS: Parking space switch is ON and text found in TA.")
	} else {
		KeywordUtil.markFailed("❌ FAIL: Parking space switch is ON but text NOT found in TA.")
	}
} else {
	if (!hasParkingText) {
		KeywordUtil.markPassed("✅ PASS: Parking space switch is OFF and text correctly not found in TA.")
	} else {
		KeywordUtil.markFailed("❌ FAIL: Parking space switch is OFF but text FOUND in TA.")
	}
}
...
WebUI.closeBrowser()
KeywordUtil.logInfo("🎯 Test completed successfully for Parking space toggle.")
