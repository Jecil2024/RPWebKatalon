...
// --------------------- DEFINE ELEMENTS ---------------------
TestObject balconySwitch = new TestObject("balconySwitch")
balconySwitch.addProperty("xpath", ConditionType.EQUALS, "//*[@id='requeststab']/div/table[3]/tbody/tr[4]/td[2]/label/div")
...
// --------------------- SCROLL PANEL TO FIND BALCONY SWITCH ---------------------
...
for (int i = 0; i < 15; i++) {
	try {
		WebElement toggleElement = driver.findElement(By.xpath("//*[@id='requeststab']/div/table[3]/tbody/tr[4]/td[2]/label/div"))
...
if (!found) {
	KeywordUtil.markFailed("❌ Balcony/Patios/Terraces switch not visible after scrolling.")
	return
}

// --------------------- TOGGLE SWITCH ---------------------
WebUI.scrollToElement(balconySwitch, 5)
WebUI.delay(2)

WebElement switchElement = WebUI.findWebElement(balconySwitch)
boolean isSwitchOn = switchElement.getAttribute("class").contains("switchOn")

if (!isSwitchOn) {
	WebUI.click(balconySwitch)
	KeywordUtil.logInfo("🔘 Balcony/Patios/Terraces switch was OFF — turned ON now.")
	isSwitchOn = true
} else {
	WebUI.click(balconySwitch)
	KeywordUtil.logInfo("🔘 Balcony/Patios/Terraces switch was ON — turned OFF now.")
	isSwitchOn = false
}
...
boolean hasBalconyText = pageSource.toLowerCase().contains("balcony, patios and terraces")

if (isSwitchOn) {
	if (hasBalconyText) {
		KeywordUtil.markPassed("✅ PASS: Balcony/Patios/Terraces switch is ON and text found in TA.")
	} else {
		KeywordUtil.markFailed("❌ FAIL: Balcony/Patios/Terraces switch is ON but text NOT found in TA.")
	}
} else {
	if (!hasBalconyText) {
		KeywordUtil.markPassed("✅ PASS: Balcony/Patios/Terraces switch is OFF and text correctly not found in TA.")
	} else {
		KeywordUtil.markFailed("❌ FAIL: Balcony/Patios/Terraces switch is OFF but text FOUND in TA.")
	}
}
...
WebUI.closeBrowser()
KeywordUtil.logInfo("🎯 Test completed successfully for Balcony/Patios/Terraces toggle.")
