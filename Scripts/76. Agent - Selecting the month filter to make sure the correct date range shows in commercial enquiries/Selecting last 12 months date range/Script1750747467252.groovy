...
// Select 12-months filter (index 2 assumed)
...
while (hasNextPage) {
    ...
    for (WebElement el : receivedDateElements) {
        ...
        if ((rawDate.isEmpty() || rawDate.contains('IDEAL MOVE IN')) || !(rawDate.matches('\\d{2}/\\d{2}/\\d{4}'))) {
            println("⏭️ Skipping invalid or non-date value: '$rawDate'")
